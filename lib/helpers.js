...
module.exports.initSettings = function(app, opts) {
  return function(next) {
    ...
    // CSRF configuration. A bit different because it's a boolean value
    var csrf = true;
    if(opts.useCSRF === false) {
        csrf = false;
    }
    else if(process.env.STORMPATH_USE_CSRF === false) {
        csrf = false;
    }
    app.set('stormpathUseCSRF', csrf);
